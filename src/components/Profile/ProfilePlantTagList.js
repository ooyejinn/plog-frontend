...
//           key={plant.id}
//           className="plant-tag"
//           style={{ marginRight: '10px', backgroundColor: selectedTags.some(tag => tag.id === plant.id && tag.type === plant.type) ? 'green' : 'white' }}
    ...
    <div className="plant-tag-list">
      {plantTags.map((plant) => (
        <button 
          key={plant.id}
          className="plant-tag"
          style={{ marginRight: '10px', backgroundColor: selectedTags.some(tag => tag.id === plant.id && tag.type === plant.type) ? '#87B989' : 'white' }}
          onClick={() => handleTagSelect(plant.id, plant.type)}
        >
          {plant.plantName}
        </button>
